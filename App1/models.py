...
class Product(models.Model):
    ...
    unit = models.CharField(max_length=50,choices=[('pkt', 'Packet')],default='pkt')
...
class PaymentTransaction(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.voucher_number:
            try:
                voucher_config = VoucherNumber.objects.get(name='Payment Voucher')
                self.voucher_number = voucher_config.get_next_voucher()
            except VoucherNumber.DoesNotExist:
                raise ValueError("VoucherNumber with name 'Payment Voucher' is not configured.")
        super().save(*args, **kwargs)

    def __str__(self):
        return f"{self.voucher_number or 'Unassigned'} - ₹{self.amount_paid}"
