...
def add_user(request):
    ...
    if not current_user:
        return redirect('login')
    roles = Role.objects.all()  # Fetch available roles for the form
    context = {
        'current_user':current_user,
        'role_permission':role_permission,
        'roles': roles
...
def add_customer11(request):
    ...
    if request.method == 'POST':
        ...
        weekday_footfall = request.POST.get('foot_fall_weekdays')
        weekend_footfall = request.POST.get('foot_fall_weekends')
        ...
        if Customer.objects.filter(gst_number=gst_number).exists():
...
def delete_daily_production(request, id):
    ...
    try:
        ...
        inventory.stock_in -= daily_production.quantity
        inventory.update_stock()
...
def send_order_email(order):
    ...
    message = (
        ...
        f"Shipping Address: {order.customer.address}\n"
        f"Billing Address: {order.customer.address}\n\n"
...
def get_customer_discount(request, customer_id):
    try:
...
def view_receipt1(request, order_id):
    current_user, role_permission = get_logged_in_user(request)
    if not current_user:
        return redirect('login')
    ...
    .select_related('product')            # fetch product for HSN
    .select_related('product')   # if you have FK to DailyProduction
)
    ...
    return render(request, 'company_admin/receipt1.html',context)
...
def fifo_batch(request, product_id, weight):
    batches = (
        ...
        .values("id", "batch_number", "manufactured_date", "expiry_date",
                "sale_price", "mrp", "stock_in", "weight_per_packet")
    )
    if not batches:
        return JsonResponse({"error": "No stock available"}, status=404)

    
    total_qty = sum(b["stock_in"] for b in batches)
